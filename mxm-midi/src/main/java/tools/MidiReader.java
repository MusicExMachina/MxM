...
class MidiReader {
    ...
    private void parseAll() {
        ...
        // For every tools.MidiTools track
        for (Track track : sequence.getTracks()) {
            ...
            // For every tools.MidiTools event
            for (int i = 0; i < track.size(); i++) {
                // Some information about this tools.MidiTools event
                MidiEvent event     = track.get(i);
                MidiMessage message = event.getMessage();
                Long tick           = event.getTick();

                if (message instanceof ShortMessage) {
                    // Cast it to a tools.MidiTools short message and parse
    ...
    private void parseShortMessage(Track track, MidiEvent event, ShortMessage message, Long tick) {
        ...
        switch (message.getCommand()) {
            ...
            case ShortMessage.PROGRAM_CHANGE:
                ...
                break;
            default:
                System.out.println("MIDI:\tUnrecognized tools.MidiTools ShortMessage " + message.getCommand() + " on channel " + message.getChannel());
                break;
        }
    }
    ...
    private void parseMetaMessage(Track track, MidiEvent event, MetaMessage message, Long tick) {
        ...
        switch(message.getType()) {
            ...
            case SEQUENCER_SPECIFIC:
                ...
                break;
            default:
                System.out.println("MIDI:\tUnrecognized tools.MidiTools MetaMessage " + message.getData());
                break;
        }
    }
    ...
    private void parseSysexMessage(Track track, MidiEvent event, SysexMessage message, Long tick) {
        System.out.println("MIDI:\tUnrecognized tools.MidiTools SysexMessage " + message.getData());
    }
